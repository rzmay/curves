...
class ObjectKeyframe extends Keyframe<JSON> {
  ...
  interpolate(keyframe: ObjectKeyframe, time: number): JSON {
    const result: JSON = {};
  ...
  protected _interpolateKey(keyframe: ObjectKeyframe, key: any, time: number): any {
    ...
    if (typeof this.value[key] === 'string' && colorString.get(this.value[key]) !== null) {
      ...
      result = colorString.to[color?.model ?? 'rgb'](
    ...
    Object.keys(this.conversionMethods).forEach((type) => {
      ...
      if (typeof this.value[key] === type) {
        const restrictedType = type as ('number' | 'string' | 'boolean' | 'object');
        const secondKeyframe: Keyframe<number | string | boolean | object> = keyframe
          .conversionMethods[restrictedType]
          .bind(keyframe)(key);
        const firstKeyframe: Keyframe<number | string | boolean | object> = this
          .conversionMethods[restrictedType]
          .bind(this)(key);

        result = firstKeyframe.interpolate(
          secondKeyframe,
          time,
          0.25,
        );
       ...
       */
        const isInt = (n: string): boolean => !Number.isNaN(parseInt(n, 10)) && parseInt(n, 10) === parseFloat(n);
        if (
          type === 'object'
